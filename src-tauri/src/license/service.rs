...
/// DeskCraft app_id registered in the codecraftgenz backend.
const APP_ID: u32 = 11;
...
/// Verifies an existing license with the remote API.
/// Sends { app_id, email, hardware_id } and checks if `valid` is true.
...
/// Sends { app_id, email, hardware_id } to the activate endpoint.
...
/// Parses the verify-license response.
...
fn parse_verify_response(text: &str, status_code: u16) -> LicenseCheckResult {
    ...
    match parsed {
        Ok(json) => {
            // Format: { "data": { "valid": bool, "message": "..." } }
...
/// Parses the activate-device response.
fn parse_activate_response(text: &str) -> LicenseCheckResult {
    let parsed: Result<serde_json::Value, _> = serde_json::from_str(text);

    match parsed {
        Ok(json) => {
            // Success: { "success": true, "data": { "license_key": "...", "message": "..." } }
            if let Some(success) = json.get("success").and_then(|s| s.as_bool()) {
                if success {
