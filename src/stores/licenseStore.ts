...
export const useLicenseStore = create<LicenseState>()((set) => ({
  ...
  checkLicense: async () => {
    ...
    try {
      const status = await tauriInvoke<LicenseStatus>('check_license', {});
      set({
  ...
  activateLicense: async (email: string) => {
    ...
    try {
      const status = await tauriInvoke<LicenseStatus>('activate_license', {
        email,
      });
      set({
        isLicensed: status.is_licensed,
  ...
  logout: async () => {
    try {
      await tauriInvoke('logout_license', {});
    } catch {
      // ignore
    }
    set({
