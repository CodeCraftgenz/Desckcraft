; DeskCraft — Inno Setup Installer Script
; Gera instalador .exe profissional para Windows
;
; Pré-requisitos:
;   1. Compilar o app: npm run tauri build
;   2. O .exe estará em: src-tauri\target\release\deskcraft.exe
...
#define MyAppDescription "Organizador inteligente de arquivos — 100% offline"

[Setup]
; Identificador único do app (GUID de produção)
AppId={{E7C3F8A1-2D9B-4E5F-A6B1-8C0D3E9F7A2B}
AppName={#MyAppName}
...
OutputDir=Output
OutputBaseFilename=DeskCraft-Setup-{#MyAppVersion}
...
begin
  ...
  begin
    if MsgBox('Deseja remover também os dados e configurações do {#MyAppName}?' + #13#10 +
              '(regras, perfis, histórico e configurações serão apagados)',
              mbConfirmation, MB_YESNO) = IDYES then
    begin
      DelTree(ExpandConstant('{localappdata}\com.deskcraft.app'), True, True, True);
    end;
  end;
end;
